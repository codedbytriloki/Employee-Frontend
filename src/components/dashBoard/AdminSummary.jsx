...
const AdminSummary = () => {
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        const summary = await axios.get(`https://employee-backend-smoky.vercel.app/api/dashboard/summary`, {
          headers: {
        ...
        alert(error.response?.data?.error || 'Error fetching summary')
      }
    }
    fetchSummary()
  }, [])
  ...
  return (
    <div className='p-6'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-4 mt-6'>
        <SummaryCard icon={<FaUsers />} text="Total Employee" number={summary.totalEmployee} color="bg-teal-600" />
